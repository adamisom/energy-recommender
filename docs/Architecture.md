# AI Energy Plan Recommender - System Architecture

## Table of Contents
1. [System Overview](#system-overview)
2. [Architecture Diagram](#architecture-diagram)
3. [Component Architecture](#component-architecture)
4. [Data Flow Diagrams](#data-flow-diagrams)
5. [AI Integration Points](#ai-integration-points)
6. [Client-Server Interactions](#client-server-interactions)
7. [Database Schema](#database-schema)
8. [API Architecture](#api-architecture)
9. [Error Handling Flows](#error-handling-flows)

---

## System Overview

### High-Level Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                        Next.js Application                       │
│                     (Single Full-Stack Codebase)                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  ┌─────────────────────┐         ┌──────────────────────┐       │
│  │   Client (Browser)  │◄───────►│  Server (Vercel)     │       │
│  │  - React Components │         │  - API Routes        │       │
│  │  - State Management │         │  - Business Logic    │       │
│  │  - Form Validation  │         │  - AI Integration    │       │
│  └─────────────────────┘         └──────────────────────┘       │
│                                            │                      │
└────────────────────────────────────────────┼──────────────────────┘
                                             │
                    ┌────────────────────────┼────────────────────────┐
                    │                        │                        │
                    ▼                        ▼                        ▼
            ┌───────────────┐      ┌─────────────────┐    ┌──────────────────┐
            │   Supabase    │      │  Anthropic API  │    │  External Data   │
            │  (PostgreSQL) │      │    (Claude)     │    │  PowerToChoose   │
            │   - Plans DB  │      │  - Explanations │    │  (Manual/Scrape) │
            │   - User DB   │      └─────────────────┘    └──────────────────┘
            └───────────────┘
```

### Technology Stack

| Layer | Technology | Purpose |
|-------|-----------|---------|
| **Frontend** | React 18 (Server Components) | UI rendering |
| **Routing** | Next.js App Router | Page routing, layouts |
| **Styling** | Tailwind CSS + shadcn/ui | Component styling |
| **Backend** | Next.js API Routes | REST API endpoints |
| **Database** | Supabase (PostgreSQL) | Plan catalog storage |
| **ORM** | Prisma | Type-safe database access |
| **AI** | Anthropic Claude API | Explanation generation |
| **Validation** | Zod | Runtime type validation |
| **Auth** | Supabase Auth (optional) | User authentication |
| **Deployment** | Vercel | Serverless hosting |

---

## Architecture Diagram

### Full System Architecture

```
┌────────────────────────────────────────────────────────────────────────┐
│                              USER BROWSER                               │
├────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌────────────┐ │
│  │  Landing     │  │   Usage      │  │ Preferences  │  │ Results    │ │
│  │  Page        │─>│   Input      │─>│   Page       │─>│  Page      │ │
│  │  (/)         │  │  (/usage)    │  │(/preferences)│  │(/recommend)│ │
│  └──────────────┘  └──────────────┘  └──────────────┘  └────────────┘ │
│                                                               │          │
│                          sessionStorage                       │          │
│                    ┌────────────────────┐                    │          │
│                    │ - usageData        │                    │          │
│                    │ - preferences      │                    │          │
│                    │ - state            │                    │          │
│                    │ - currentPlan      │                    │          │
│                    └────────────────────┘                    │          │
│                                                               │          │
└───────────────────────────────────────────────────────────────┼──────────┘
                                                                │
                                                       POST /api/recommendations
                                                                │
┌───────────────────────────────────────────────────────────────┼──────────┐
│                          VERCEL EDGE NETWORK                   │          │
├────────────────────────────────────────────────────────────────┼─────────┤
│                                                                 ▼         │
│  ┌────────────────────────────────────────────────────────────────────┐ │
│  │              API Route: /api/recommendations                       │ │
│  │  ┌──────────────────────────────────────────────────────────────┐ │ │
│  │  │ 1. Rate Limiting (IP-based)                                  │ │ │
│  │  │ 2. Request Validation (Zod)                                  │ │ │
│  │  │ 3. Fetch Plans (Prisma → Supabase)                          │ │ │
│  │  │ 4. Analyze Usage Patterns                                    │ │ │
│  │  │ 5. Calculate Costs (all plans)                              │ │ │
│  │  │ 6. Score Plans (custom algorithm)                           │ │ │
│  │  │ 7. Rank & Filter (top 3)                                    │ │ │
│  │  │ 8. Generate Explanations (AI) ◄──────────┐                 │ │ │
│  │  │ 9. Build Response                        │                 │ │ │
│  │  └──────────────────────────────────────────┼─────────────────┘ │ │
│  └───────────────────────────────────────────────┼───────────────────┘ │
│                                                   │                     │
│  ┌────────────────────────────────────────────────────────────────────┐ │
│  │                    Business Logic Layer                            │ │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────────┐│ │
│  │  │   Scoring    │  │     Cost     │  │    Usage Analysis        ││ │
│  │  │   Engine     │  │  Calculator  │  │  (pattern detection)     ││ │
│  │  └──────────────┘  └──────────────┘  └──────────────────────────┘│ │
│  └────────────────────────────────────────────────────────────────────┘ │
│                                                   │                     │
└───────────────────────────────────────────────────┼─────────────────────┘
                                                    │
                            ┌───────────────────────┼───────────────────┐
                            │                       │                   │
                            ▼                       ▼                   ▼
                    ┌───────────────┐    ┌──────────────────┐  ┌─────────────┐
                    │   Supabase    │    │  Anthropic API   │  │   Cache     │
                    │  (PostgreSQL) │    │  (Claude 3.5)    │  │ (In-Memory) │
                    │               │    │                  │  │ - LLM resp  │
                    │  Tables:      │    │  Model:          │  │ - Rate lim  │
                    │  - plans      │    │  Sonnet 4        │  └─────────────┘
                    │  - users      │    │                  │
                    └───────────────┘    └──────────────────┘
```

---

## Component Architecture

### Frontend Components (Client-Side)

```
app/
├── page.tsx (Server Component)
│   └── Landing Page
│       - Hero section
│       - How it works
│       - CTA button
│
├── usage/page.tsx (Client Component)
│   └── Usage Input Form
│       - Manual entry (12 inputs)
│       - CSV upload
│       - Validation
│       └─> sessionStorage
│
├── preferences/page.tsx (Client Component)
│   └── Preferences Form
│       - State selection
│       - Priority selection
│       - Advanced filters (sliders)
│       - Current plan (optional)
│       └─> sessionStorage
│
├── recommendations/page.tsx (Client Component)
│   └── Results Display
│       - Loading state
│       - Top 3 cards
│       - AI explanations
│       - Sign up buttons
│       └─> API call
│
└── plan/[id]/page.tsx (Server Component)
    └── Plan Details
        - Fetch from DB
        - Full plan info
        - Sign up modal
```

### Backend Components (Server-Side)

```
lib/
├── scoring/
│   ├── usage-analysis.ts
│   │   └── analyzeUsage()
│   │       - Pattern detection
│   │       - Statistical analysis
│   │       - Peak identification
│   │
│   ├── cost-calculator.ts
│   │   └── calculatePlanCost()
│   │       - Fixed rate calc
│   │       - Variable rate calc
│   │       - TOU calc (40/60 split)
│   │       - ETF calculation
│   │
│   ├── plan-scorer.ts
│   │   └── scorePlan()
│   │       - Cost score (0-100)
│   │       - Flexibility score
│   │       - Renewable score
│   │       - Rating score
│   │       - Seasonal score (feature-flagged)
│   │       - Weighted combination
│   │
│   └── plan-ranker.ts
│       └── filterAndRankPlans()
│           - Filter by constraints
│           - Relax if <3 plans
│           - Sort by score
│
├── anthropic/
│   ├── client.ts
│   │   └── Claude API client
│   │
│   ├── explanations.ts
│   │   └── generateExplanation()
│   │       - Build prompt
│   │       - Call Claude API
│   │       - Validate response
│   │       - Fallback template
│   │       - Cache results
│   │
│   └── batch-explanations.ts
│       └── generateAllExplanations()
│           - Parallel API calls (3x)
│           - Promise.all()
│
└── database/
    ├── client.ts
    │   └── Prisma singleton
    │
    └── recs-parser.ts
        └── Parse test data
```

---

## Data Flow Diagrams

### Flow 1: Complete User Journey

```
┌──────────┐
│  User    │
└────┬─────┘
     │
     ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 1: Landing Page                                             │
│ Component: app/page.tsx (Server)                                 │
│ Action: Click "Get Started"                                      │
└────┬────────────────────────────────────────────────────────────┘
     │ Navigate to /usage
     ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 2: Usage Input                                              │
│ Component: app/usage/page.tsx (Client)                           │
│ ┌─────────────────────────────────────────────────────────────┐ │
│ │ User Actions:                                                │ │
│ │ - Upload CSV OR manually enter 12 months                     │ │
│ │ - Validate: all positive, < 10,000 kWh                      │ │
│ │                                                               │ │
│ │ Data Storage:                                                 │ │
│ │ sessionStorage.setItem('usageData', JSON.stringify(data))    │ │
│ └─────────────────────────────────────────────────────────────┘ │
└────┬────────────────────────────────────────────────────────────┘
     │ Navigate to /preferences
     ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 3: Preferences                                              │
│ Component: app/preferences/page.tsx (Client)                     │
│ ┌─────────────────────────────────────────────────────────────┐ │
│ │ User Actions:                                                │ │
│ │ - Select state (TX, PA, OH, IL)                             │ │
│ │ - Select priority (cost, renewable, flexibility, balanced)   │ │
│ │ - Set advanced filters (sliders)                             │ │
│ │ - Optional: select current plan                              │ │
│ │                                                               │ │
│ │ Data Storage:                                                 │ │
│ │ sessionStorage.setItem('preferences', JSON.stringify(prefs)) │ │
│ │ sessionStorage.setItem('state', state)                       │ │
│ │ sessionStorage.setItem('currentPlan', planId)                │ │
│ └─────────────────────────────────────────────────────────────┘ │
└────┬────────────────────────────────────────────────────────────┘
     │ Navigate to /recommendations
     ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 4: Generate Recommendations (API Call)                      │
│ Component: app/recommendations/page.tsx (Client)                 │
│                                                                   │
│ ┌─────────────────────────────────────────────────────────────┐ │
│ │ useEffect on mount:                                          │ │
│ │ 1. Read from sessionStorage                                  │ │
│ │ 2. Build API request                                         │ │
│ │ 3. POST /api/recommendations                                 │ │
│ │ 4. Show loading state                                        │ │
│ │ 5. Display results                                           │ │
│ └─────────────────────────────────────────────────────────────┘ │
└────┬────────────────────────────────────────────────────────────┘
     │ API Request
     ▼
┌─────────────────────────────────────────────────────────────────┐
│ SERVER-SIDE PROCESSING                                           │
│ (See detailed API flow below)                                    │
│ - Fetch plans from DB                                            │
│ - Run scoring algorithm                                          │
│ - Generate AI explanations                                       │
│ - Return top 3 recommendations                                   │
└────┬────────────────────────────────────────────────────────────┘
     │ API Response
     ▼
┌─────────────────────────────────────────────────────────────────┐
│ Step 5: Display Results                                          │
│ Component: app/recommendations/page.tsx (Client)                 │
│ - Show top 3 cards with explanations                             │
│ - "View Details" → /plan/[id]                                    │
│ - "Sign Up" → Modal (MVP limitation)                             │
│ - "Try Different Preferences" → /preferences                     │
│ - "Start Over" → /usage (clear sessionStorage)                   │
└──────────────────────────────────────────────────────────────────┘
```

---

## Component Architecture

### Client-Server Boundary

```
┌─────────────────────────────────────────────────────────────────────┐
│                         CLIENT (Browser)                             │
├─────────────────────────────────────────────────────────────────────┤
│                                                                       │
│  React Components ('use client')                                     │
│  ├── usage/page.tsx          [State: usageData]                     │
│  ├── preferences/page.tsx     [State: preferences, state]            │
│  └── recommendations/page.tsx [State: loading, error, recommendations]│
│                                                                       │
│  State Management: useState + sessionStorage                         │
│  Form Validation: Client-side (before API call)                      │
│  API Calls: fetch() to /api/*                                        │
│                                                                       │
└─────────────────────────────┬───────────────────────────────────────┘
                              │ HTTP/HTTPS
                              │ JSON payloads
                              │
┌─────────────────────────────┴───────────────────────────────────────┐
│                         SERVER (Vercel Edge)                         │
├─────────────────────────────────────────────────────────────────────┤
│                                                                       │
│  API Routes (Next.js Route Handlers)                                 │
│  ├── /api/recommendations/route.ts  [POST]                          │
│  ├── /api/plans/route.ts            [GET]                           │
│  └── /api/plans/[id]/route.ts       [GET]                           │
│                                                                       │
│  Server Components                                                   │
│  ├── page.tsx (landing)                                              │
│  └── plan/[id]/page.tsx (fetches from DB directly)                  │
│                                                                       │
│  Business Logic (Pure Functions)                                     │
│  ├── lib/scoring/*         [Scoring algorithms]                     │
│  ├── lib/anthropic/*       [AI integration]                         │
│  └── lib/database/*        [Data access]                            │
│                                                                       │
└─────────────────────────────┬──────────────┬────────────────────────┘
                              │              │
                    ┌─────────┴──────┐  ┌────┴──────────┐
                    │   Supabase     │  │  Claude API   │
                    │   PostgreSQL   │  │  (Anthropic)  │
                    └────────────────┘  └───────────────┘
```

---

## Data Flow Diagrams

### Primary Flow: Recommendation Generation

```
CLIENT                          SERVER                       EXTERNAL SERVICES
  │                               │                                │
  │ 1. POST /api/recommendations  │                                │
  ├─────────────────────────────>│                                │
  │                               │                                │
  │                               │ 2. Rate Limit Check            │
  │                               │    (in-memory store)           │
  │                               │                                │
  │                               │ 3. Validate Request (Zod)      │
  │                               │                                │
  │                               │ 4. Query Plans                 │
  │                               ├───────────────────────────────>│
  │                               │    SELECT * FROM plans         │ Supabase
  │                               │    WHERE state = ?             │
  │                               │<───────────────────────────────┤
  │                               │    [20-50 plan records]        │
  │                               │                                │
  │                               │ 5. Analyze Usage               │
  │                               │    - Calculate averages        │
  │                               │    - Detect patterns           │
  │                               │    - Identify peaks            │
  │                               │                                │
  │                               │ 6. Calculate Costs (all plans) │
  │                               │    for (plan of plans) {       │
  │                               │      cost = calculateCost()    │
  │                               │    }                           │
  │                               │                                │
  │                               │ 7. Score Plans                 │
  │                               │    - Cost score                │
  │                               │    - Flexibility score         │
  │                               │    - Renewable score           │
  │                               │    - Rating score              │
  │                               │    - Seasonal score (optional) │
  │                               │    - Weighted sum              │
  │                               │                                │
  │                               │ 8. Rank & Filter               │
  │                               │    - Sort by finalScore        │
  │                               │    - Take top 3                │
  │                               │                                │
  │                               │ 9. Generate Explanations (AI)  │
  │                               │    Promise.all([               │
  │                               │      explain(plan1),  ────────>│
  │                               │      explain(plan2),  ────────>│ Claude API
  │                               │      explain(plan3)   ────────>│ (3 parallel)
  │                               │    ])                          │
  │                               │<───────────────────────────────┤
  │                               │    [3 explanations]            │
  │                               │                                │
  │                               │ 10. Check Cache First          │
  │                               │     (skip API if cached)       │
  │                               │                                │
  │                               │ 11. Build Response             │
  │                               │     - recommendations (3)      │
  │                               │     - metadata                 │
  │                               │     - confidence level         │
  │                               │                                │
  │ <─────────────────────────────┤                                │
  │  JSON Response                │                                │
  │  {                            │                                │
  │    recommendations: [...],    │                                │
  │    metadata: {...}            │                                │
  │  }                            │                                │
  │                               │                                │
  │ 12. Display Results           │                                │
  │     - Render cards            │                                │
  │     - Show explanations       │                                │
  │                               │                                │
```

### Data Flow Metrics

| Step | Processing Time | Bottleneck? |
|------|-----------------|-------------|
| 1-2. Rate limit + validation | <10ms | No |
| 3-4. Database query | 50-100ms | No |
| 5. Usage analysis | <5ms | No |
| 6. Cost calculation (20 plans) | <10ms | No |
| 7. Plan scoring (20 plans) | <10ms | No |
| 8. Ranking | <5ms | No |
| 9. AI explanations (3 calls) | 1-2 seconds | ⚠️ **Yes** |
| 10. Response serialization | <5ms | No |
| **Total** | **1-2.5 seconds** | Target: <2s |

---

## AI Integration Points

### 🤖 AI Usage: Explanation Generation

#### Where AI is Used

```
┌────────────────────────────────────────────────────────────┐
│                   AI Integration Layer                      │
├────────────────────────────────────────────────────────────┤
│                                                              │
│  lib/anthropic/explanations.ts                              │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ generateExplanation(context)                         │  │
│  │                                                       │  │
│  │ Input Context:                                        │  │
│  │ ├── Plan details (name, rate, contract, etc.)       │  │
│  │ ├── User profile (usage, pattern, priorities)       │  │
│  │ ├── Cost breakdown (annual, savings, etc.)          │  │
│  │ └── Ranking (why this plan? #1, #2, or #3)         │  │
│  │                                                       │  │
│  │ Process:                                              │  │
│  │ 1. Check cache (state+planId+rank+priority)         │  │
│  │ 2. If cached → return immediately                    │  │
│  │ 3. If not cached → call Claude API                   │  │
│  │                                                       │  │
│  │ AI Prompt Structure:                                  │  │
│  │ ┌─────────────────────────────────────────────────┐ │  │
│  │ │ "You are an expert energy plan advisor..."      │ │  │
│  │ │                                                   │ │  │
│  │ │ User Profile: [usage, pattern, priorities]      │ │  │
│  │ │ Recommended Plan: [all details]                 │ │  │
│  │ │                                                   │ │  │
│  │ │ Why ranked #X? Focus on user priorities.        │ │  │
│  │ │ Requirements: 2-3 sentences, specific numbers    │ │  │
│  │ └─────────────────────────────────────────────────┘ │  │
│  │                                                       │  │
│  │ API Call:                                             │  │
│  │ - Model: claude-3-5-sonnet-20241022                 │  │
│  │ - Max tokens: 300                                    │  │
│  │ - Timeout: 10 seconds                                │  │
│  │                                                       │  │
│  │ Validation:                                           │  │
│  │ - Length: 50-500 chars                               │  │
│  │ - Contains numbers                                    │  │
│  │ - Not generic ("great option")                       │  │
│  │                                                       │  │
│  │ Fallback:                                             │  │
│  │ - On error → template-based explanation             │  │
│  │ - On timeout → template-based explanation           │  │
│  │ - On validation failure → template                   │  │
│  │                                                       │  │
│  │ Cache Result:                                         │  │
│  │ explanationCache.set(key, explanation)               │  │
│  │ (max 1000 entries, FIFO eviction)                   │  │
│  └──────────────────────────────────────────────────────┘  │
└────────────────────────────────────────────────────────────┘
```

#### AI Prompt Example

```
Input to Claude:
┌─────────────────────────────────────────────────────────────┐
│ You are an expert energy plan advisor. Explain why this     │
│ plan was recommended in 2-3 clear, specific sentences.      │
│                                                              │
│ User Profile:                                                │
│ - Annual usage: 12,000 kWh (1,000 kWh/month average)       │
│ - Usage pattern: summer peak                                 │
│ - Current annual cost: $1,450                               │
│ - Priorities: cost (renewable: 0%+, max contract: 24 mo)    │
│                                                              │
│ Recommended Plan (Rank #1):                                  │
│ - Supplier: Texas GreenPower                                │
│ - Plan: 100% Green 12-Month                                 │
│ - Rate: $0.1199/kWh                                         │
│ - Monthly fee: $9.95                                        │
│ - Contract: 12 months                                        │
│ - Renewable: 100%                                            │
│ - Supplier rating: 4.3/5.0                                  │
│ - Projected annual cost: $1,558.80                          │
│ - Savings: -$108.80/year (more expensive)                   │
│                                                              │
│ Why is this plan ranked #1? Focus on what matters most.     │
└─────────────────────────────────────────────────────────────┘

Output from Claude:
┌─────────────────────────────────────────────────────────────┐
│ While this plan costs $108.80 more annually, it provides    │
│ 100% renewable energy with a strong 4.3/5.0 supplier rating.│
│ The 12-month contract offers rate stability during your     │
│ summer peak months when your usage spikes to 1,450+ kWh.   │
└─────────────────────────────────────────────────────────────┘
```

#### AI Performance Optimization

```
Optimization Strategy:
┌────────────────────────────────────────────────────┐
│ 1. Caching (Most Important)                        │
│    - Key: state-planId-rank-priority              │
│    - Hit rate: ~70-80% (common plans)             │
│    - Saves: ~$0.03 per cached request             │
│                                                     │
│ 2. Parallel Calls                                  │
│    - Promise.all([explain1, explain2, explain3])  │
│    - Time saved: ~2-3 seconds vs sequential       │
│                                                     │
│ 3. Fallback Templates                              │
│    - On error: 0ms, $0 cost                       │
│    - Quality: Good enough for MVP                  │
│                                                     │
│ 4. Token Limits                                    │
│    - Input: ~400 tokens                            │
│    - Output: max 300 tokens                        │
│    - Cost: ~$0.003 per explanation (3 × $0.001)  │
│                                                     │
│ 5. Timeout Protection                              │
│    - 10 second limit                               │
│    - Prevents hanging requests                     │
└────────────────────────────────────────────────────┘

Cost per recommendation request:
- No cache hits: 3 explanations × $0.001 = $0.003
- With 70% hit rate: 1 explanation × $0.001 = $0.001
- Fallback (error): $0.000

Expected: 100 requests/day × $0.001 avg = $0.10/day = $3/month
```

---

## Client-Server Interactions

### Interaction Pattern: RESTful API

```
CLIENT ACTIONS                  SERVER RESPONSES
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

1. Load Landing Page
   GET /
   ────────────────────────────>
                                 HTML (Server Component)
   <────────────────────────────
                                 Status: 200

2. Load Usage Page
   GET /usage
   ────────────────────────────>
                                 HTML (Client Component bundle)
   <────────────────────────────
                                 Status: 200

3. Submit Usage Data
   (No server call - saved to sessionStorage)

4. Load Preferences Page
   GET /preferences
   ────────────────────────────>
                                 HTML (Client Component bundle)
   <────────────────────────────
                                 Status: 200

5. Submit Preferences
   (No server call - saved to sessionStorage)

6. Get Recommendations
   POST /api/recommendations
   {
     userId: "user-123",
     state: "TX",
     monthlyUsageKwh: [850, ...],
     preferences: {...}
   }
   ────────────────────────────>
                                 Process request:
                                 - Rate limit check
                                 - Validate with Zod
                                 - Query database
                                 - Run algorithms
                                 - Call Claude API (3×)
                                 - Build response
   <────────────────────────────
   {                             Status: 200
     recommendations: [...],     Time: 1-2 seconds
     metadata: {...}
   }

7. View Plan Details
   GET /plan/[id]
   ────────────────────────────>
                                 HTML (Server Component)
                                 - Fetch from DB
                                 - Render page
   <────────────────────────────
                                 Status: 200

8. Get Plan List (Optional)
   GET /api/plans?state=TX
   ────────────────────────────>
                                 Query database
                                 Filter by params
   <────────────────────────────
   {                             Status: 200
     plans: [...],
     count: 17
   }
```

### Request/Response Contracts

#### POST /api/recommendations

**Request:**
```typescript
{
  userId: string                    // "user-123" (placeholder for MVP)
  state?: 'TX' | 'PA' | 'OH' | 'IL' // Filter plans by state
  monthlyUsageKwh: number[12]       // Exactly 12 months
  currentPlan?: {
    planId: string                  // Plan identifier
    startDate?: string              // ISO date (optional)
    contractEndDate?: string        // ISO date (optional)
  }
  preferences: {
    priority: 'cost' | 'renewable' | 'flexibility' | 'balanced'
    minRenewablePct: number         // 0-100
    maxContractMonths: number       // 1-36
    minSupplierRating: number       // 1.0-5.0
  }
}
```

**Response (Success - 200):**
```typescript
{
  recommendations: [
    {
      rank: 1,
      plan: { /* full plan object */ },
      projectedAnnualCost: 1558.80,
      annualSavings: -108.80,
      explanation: "While this plan costs...", // AI-generated
      score: 87.5,
      breakdown: {
        costScore: 72,
        flexibilityScore: 50,
        renewableScore: 100,
        ratingScore: 86,
        seasonalScore: 60,
        finalScore: 87.5
      }
    },
    // ... 2 more
  ],
  metadata: {
    totalAnnualUsageKwh: 12000,
    usagePattern: 'summer_peak',
    currentPlanAnnualCost: 1450,
    generatedAt: "2025-11-11T...",
    confidence: 'high'
  }
}
```

**Response (Error - 400/429/500):**
```typescript
{
  error: string,              // Human-readable message
  details?: ZodError[]        // Validation details (400 only)
}

Headers (429):
  X-RateLimit-Limit: "10"
  X-RateLimit-Remaining: "0"
  X-RateLimit-Reset: "1699564800000"
```

---

## Database Schema

### Entity Relationship Diagram

```
┌─────────────────────────────────────────────────────────┐
│                        Plan                              │
├─────────────────────────────────────────────────────────┤
│ PK  id                    String (cuid)                  │
│ UQ  planId                String                         │
│ IDX state                 String ('TX', 'PA', ...)       │
│ IDX supplierName          String                         │
│     planName              String                         │
│     rateType              String ('fixed', 'variable')   │
│ IDX ratePerKwh            Float                          │
│     monthlyFee            Float                          │
│     contractLengthMonths  Int?                           │
│     earlyTerminationFee   Float                          │
│ IDX renewablePct          Int                            │
│     supplierRating        Float                          │
│     planDetails           Json?                          │
│     createdAt             DateTime                       │
│     updatedAt             DateTime                       │
└─────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────┐
│                        User                              │
│                    (Optional for MVP)                    │
├─────────────────────────────────────────────────────────┤
│ PK  id                    String (cuid)                  │
│ UQ  email                 String                         │
│     password              String (hashed)                │
│     name                  String?                        │
│     createdAt             DateTime                       │
│     updatedAt             DateTime                       │
└─────────────────────────────────────────────────────────┘

Note: No foreign key relationships in MVP
      (users don't save recommendations)
```

### Database Queries

```sql
-- Most common query (Recommendations API)
SELECT * FROM plans 
WHERE state = 'TX'
  AND renewablePct >= 0
  AND contractLengthMonths <= 24
  AND supplierRating >= 3.0
ORDER BY supplierRating DESC;

-- Indexes support:
-- 1. state (primary filter)
-- 2. renewablePct (common constraint)
-- 3. ratePerKwh (cost comparisons)
-- 4. supplierName (plan lookups)

-- Query performance: <100ms for 20-50 plans
```

---

## API Architecture

### API Route Organization

```
app/api/
├── recommendations/
│   └── route.ts                    [POST]
│       - Rate limiting (10 req/min)
│       - Request validation (Zod)
│       - Main recommendation logic
│       - AI explanation generation
│       - Response: Top 3 recommendations
│
└── plans/
    ├── route.ts                    [GET]
    │   - List all plans (with filters)
    │   - Query params: state, minRenewable, maxContract, etc.
    │   - Response: Plan array
    │
    └── [id]/
        └── route.ts                [GET]
            - Get single plan by ID
            - Response: Plan object or 404
```

### API Processing Flow (Recommendations)

```
Request arrives
     │
     ▼
┌─────────────────────────────────────────────────────────┐
│ 1. RATE LIMITING                                         │
│    ┌──────────────────────────────────────────────┐    │
│    │ Check: IP address                             │    │
│    │ Limit: 10 requests per 60 seconds            │    │
│    │ Store: In-memory Map (dev only)              │    │
│    │                                               │    │
│    │ If exceeded:                                  │    │
│    │   return 429 Too Many Requests               │    │
│    └──────────────────────────────────────────────┘    │
└────┬────────────────────────────────────────────────────┘
     │ PASS
     ▼
┌─────────────────────────────────────────────────────────┐
│ 2. INPUT VALIDATION (Zod Schema)                         │
│    ┌──────────────────────────────────────────────┐    │
│    │ Validate:                                     │    │
│    │ - userId: string ✓                           │    │
│    │ - state: enum ✓                              │    │
│    │ - monthlyUsageKwh: array[12] of numbers ✓   │    │
│    │ - preferences: object with constraints ✓     │    │
│    │                                               │    │
│    │ If invalid:                                   │    │
│    │   return 400 Bad Request + error details    │    │
│    └──────────────────────────────────────────────┘    │
└────┬────────────────────────────────────────────────────┘
     │ VALID
     ▼
┌─────────────────────────────────────────────────────────┐
│ 3. DATABASE QUERY (Prisma + Supabase)                   │
│    ┌──────────────────────────────────────────────┐    │
│    │ Query: prisma.plan.findMany({                │    │
│    │   where: { state: validatedData.state }      │    │
│    │ })                                            │    │
│    │                                               │    │
│    │ Returns: 15-20 plan records for state        │    │
│    │ Time: 50-100ms                               │    │
│    │                                               │    │
│    │ If empty:                                     │    │
│    │   return 503 Service Unavailable             │    │
│    └──────────────────────────────────────────────┘    │
└────┬────────────────────────────────────────────────────┘
     │ PLANS FETCHED
     ▼
┌─────────────────────────────────────────────────────────┐
│ 4. BUSINESS LOGIC (Pure Functions)                      │
│    ┌──────────────────────────────────────────────┐    │
│    │ a) analyzeUsage(monthlyUsageKwh)             │    │
│    │    - Calculate averages, median              │    │
│    │    - Detect pattern (summer/winter/flat)     │    │
│    │    - Identify peak months                    │    │
│    │    Time: <5ms                                │    │
│    │                                               │    │
│    │ b) calculatePlanCost(plan, usage, ...)       │    │
│    │    - For each plan (20×):                    │    │
│    │      * Calculate energy charges              │    │
│    │      * Add monthly fees                      │    │
│    │      * Add switching costs (if applicable)   │    │
│    │    Time: <10ms total                         │    │
│    │                                               │    │
│    │ c) scorePlan(plan, cost, preferences, ...)   │    │
│    │    - For each plan (20×):                    │
│    │      * Cost score (0-100)                    │    │
│    │      * Flexibility score (0-100)             │    │
│    │      * Renewable score (0-100)               │    │
│    │      * Rating score (0-100)                  │    │
│    │      * Seasonal score (0-100, optional)      │    │
│    │      * Weighted sum → finalScore             │    │
│    │    Time: <10ms total                         │    │
│    │                                               │    │
│    │ d) filterAndRankPlans(...)                   │    │
│    │    - Filter by constraints                   │    │
│    │    - Relax if <3 matches                     │    │
│    │    - Sort by finalScore DESC                 │    │
│    │    - Return top 3                            │    │
│    │    Time: <5ms                                │    │
│    └──────────────────────────────────────────────┘    │
└────┬────────────────────────────────────────────────────┘
     │ TOP 3 PLANS
     ▼
┌─────────────────────────────────────────────────────────┐
│ 5. AI EXPLANATION GENERATION (Anthropic Claude)         │
│    ┌──────────────────────────────────────────────┐    │
│    │ 🤖 For each of top 3 plans (parallel):       │    │
│    │                                               │    │
│    │ generateExplanationWithCache(context)        │    │
│    │ ├─ Check cache first                         │    │
│    │ │  └─ If hit: return cached (0ms)           │    │
│    │ │                                             │    │
│    │ └─ If miss: Call Claude API                  │    │
│    │    ├─ Build prompt (400 tokens)              │    │
│    │    ├─ API call (with 10s timeout)            │    │
│    │    ├─ Validate response                      │    │
│    │    ├─ Cache result                           │    │
│    │    └─ Return explanation                     │    │
│    │                                               │    │
│    │ Time: 500ms-2s per call (parallel)          │    │
│    │ Total: 1-2 seconds for all 3                │    │
│    │                                               │    │
│    │ Error handling:                               │    │
│    │ - Timeout → Use template                     │    │
│    │ - API error → Use template                   │    │
│    │ - Invalid → Use template                     │    │
│    └──────────────────────────────────────────────┘    │
└────┬────────────────────────────────────────────────────┘
     │ EXPLANATIONS READY
     ▼
┌─────────────────────────────────────────────────────────┐
│ 6. RESPONSE CONSTRUCTION                                 │
│    ┌──────────────────────────────────────────────┐    │
│    │ Build RecommendationResponse:                │    │
│    │ - Map scored plans to recommendations        │    │
│    │ - Attach AI explanations                     │    │
│    │ - Calculate confidence level                 │    │
│    │ - Add metadata                               │    │
│    │                                               │    │
│    │ Serialize to JSON                            │    │
│    │ Time: <5ms                                   │    │
│    └──────────────────────────────────────────────┘    │
└────┬────────────────────────────────────────────────────┘
     │
     ▼
Response sent to client
Status: 200 OK
Content-Type: application/json
Total time: 1-2.5 seconds
```

### State Management Pattern

```
CLIENT STATE FLOW:

sessionStorage (Persistent Across Pages)
┌────────────────────────────────────────────┐
│ Key             │ Value Type                │
├────────────────────────────────────────────┤
│ usageData       │ number[] (12 months)     │
│ preferences     │ UserPreferences object   │
│ state           │ 'TX' | 'PA' | 'OH' | 'IL'│
│ currentPlan     │ string (planId)          │
└────────────────────────────────────────────┘

Component State (React useState)
┌────────────────────────────────────────────┐
│ Page            │ Local State              │
├────────────────────────────────────────────┤
│ usage/page      │ - usageData (temp)       │
│                 │ - errors                 │
│                 │                          │
│ preferences/    │ - preferences (temp)     │
│ page            │ - state (temp)           │
│                 │ - currentPlan (temp)     │
│                 │                          │
│ recommendations/│ - loading                │
│ page            │ - error                  │
│                 │ - recommendations        │
└────────────────────────────────────────────┘

Data Flow Between Pages:
usage/page.tsx
    │ sessionStorage.setItem('usageData', ...)
    ▼
preferences/page.tsx
    │ sessionStorage.getItem('usageData')
    │ sessionStorage.setItem('preferences', ...)
    │ sessionStorage.setItem('state', ...)
    ▼
recommendations/page.tsx
    │ sessionStorage.getItem('usageData')
    │ sessionStorage.getItem('preferences')
    │ sessionStorage.getItem('state')
    ▼
API call with combined data
```

---

## Detailed Process Flows

### Scoring Algorithm Flow

```
Input: Plan, Usage, Preferences, All Costs
     │
     ▼
┌──────────────────────────────────────────────────────────┐
│ NORMALIZE COST SCORE (0-100)                              │
│ ┌────────────────────────────────────────────────────┐   │
│ │ maxCost = max(allCosts.map(c => c.firstYearTotal)) │   │
│ │ costScore = 100 × (1 - thisCost/maxCost)           │   │
│ │                                                      │   │
│ │ Example:                                             │   │
│ │ - Plan A: $1200/yr → score: 100 (cheapest)         │   │
│ │ - Plan B: $1400/yr → score: 85.7                   │   │
│ │ - Plan C: $1800/yr → score: 33.3 (most expensive)  │   │
│ └────────────────────────────────────────────────────┘   │
└────┬─────────────────────────────────────────────────────┘
     │
     ▼
┌──────────────────────────────────────────────────────────┐
│ FLEXIBILITY SCORE (0-100)                                 │
│ ┌────────────────────────────────────────────────────┐   │
│ │ Based on contract length:                          │   │
│ │ - Month-to-month: 100                              │   │
│ │ - ≤6 months: 70                                    │   │
│ │ - ≤12 months: 50                                   │   │
│ │ - ≤24 months: 30                                   │   │
│ │ - >24 months: 10                                   │   │
│ └────────────────────────────────────────────────────┘   │
└────┬─────────────────────────────────────────────────────┘
     │
     ▼
┌──────────────────────────────────────────────────────────┐
│ RENEWABLE SCORE (0-100)                                   │
│ ┌────────────────────────────────────────────────────┐   │
│ │ Direct mapping:                                    │   │
│ │ renewableScore = renewablePct                      │   │
│ │                                                     │   │
│ │ - 100% renewable → 100                             │   │
│ │ - 50% renewable → 50                               │   │
│ │ - 0% renewable → 0                                 │   │
│ └────────────────────────────────────────────────────┘   │
└────┬─────────────────────────────────────────────────────┘
     │
     ▼
┌──────────────────────────────────────────────────────────┐
│ SUPPLIER RATING SCORE (0-100)                             │
│ ┌────────────────────────────────────────────────────┐   │
│ │ ratingScore = (supplierRating / 5.0) × 100         │   │
│ │                                                     │   │
│ │ - 5.0/5.0 → 100                                    │   │
│ │ - 4.0/5.0 → 80                                     │   │
│ │ - 3.0/5.0 → 60                                     │   │
│ └────────────────────────────────────────────────────┘   │
└────┬─────────────────────────────────────────────────────┘
     │
     ▼
┌──────────────────────────────────────────────────────────┐
│ SEASONAL FIT SCORE (0-100) [Feature-Flagged]             │
│ ┌────────────────────────────────────────────────────┐   │
│ │ If ENABLE_SEASONAL_SCORING = false:                │   │
│ │   return 50 (neutral)                              │   │
│ │                                                     │   │
│ │ If enabled, based on rateType × usagePattern:     │   │
│ │ ┌────────────────────────────────────────────┐    │   │
│ │ │ TOU + summer_peak → 75                     │    │   │
│ │ │ TOU + winter_peak → 40                     │    │   │
│ │ │ Fixed (any pattern) → 60                   │    │   │
│ │ │ Variable + peak → 40                       │    │   │
│ │ │ Variable + flat → 55                       │    │   │
│ │ └────────────────────────────────────────────┘    │   │
│ └────────────────────────────────────────────────────┘   │
└────┬─────────────────────────────────────────────────────┘
     │
     ▼
┌──────────────────────────────────────────────────────────┐
│ WEIGHTED COMBINATION                                      │
│ ┌────────────────────────────────────────────────────┐   │
│ │ Get weights based on user priority:               │   │
│ │                                                     │   │
│ │ if priority = 'cost':                              │   │
│ │   weights = {                                      │   │
│ │     cost: 0.60, flexibility: 0.10,                │   │
│ │     renewable: 0.10, rating: 0.10, seasonal: 0.10 │   │
│ │   }                                                │   │
│ │                                                     │   │
│ │ if priority = 'renewable':                         │   │
│ │   weights = { renewable: 0.50, cost: 0.30, ... }  │   │
│ │                                                     │   │
│ │ finalScore =                                       │   │
│ │   (costScore × weights.cost) +                    │   │
│ │   (flexScore × weights.flexibility) +             │   │
│ │   (renewScore × weights.renewable) +              │   │
│ │   (ratingScore × weights.rating) +                │   │
│ │   (seasonalScore × weights.seasonal)              │   │
│ │                                                     │   │
│ │ Example:                                           │   │
│ │ - Cost priority: 60% weight on cost score         │   │
│ │ - Renewable priority: 50% weight on renewable     │   │
│ └────────────────────────────────────────────────────┘   │
└────┬─────────────────────────────────────────────────────┘
     │
     ▼
   Return ScoreBreakdown
   {
     costScore: 85.7,
     flexibilityScore: 50,
     renewableScore: 100,
     ratingScore: 86,
     seasonalScore: 60,
     finalScore: 87.5
   }
```

### AI Explanation Generation Flow

```
Input: Top 3 Scored Plans
     │
     ▼
┌────────────────────────────────────────────────────────────┐
│ FOR EACH PLAN (Parallel Processing)                        │
├────────────────────────────────────────────────────────────┤
│                                                              │
│  Plan #1              Plan #2              Plan #3          │
│     │                    │                    │             │
│     ▼                    ▼                    ▼             │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 1. Build Cache Key                                    │  │
│  │    key = `${state}-${planId}-${rank}-${priority}`   │  │
│  └────┬─────────────────────────────────────────────────┘  │
│       │                                                     │
│       ▼                                                     │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 2. Check Cache                                        │  │
│  │    ┌────────────┐                                     │  │
│  │    │ Cache Hit? │ YES → Return cached explanation     │  │
│  │    └────┬───────┘                                     │  │
│  │         │ NO                                           │  │
│  └─────────┼──────────────────────────────────────────────┘  │
│            ▼                                                  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 3. Build AI Prompt                                    │  │
│  │    ┌──────────────────────────────────────────────┐  │  │
│  │    │ Context includes:                             │  │  │
│  │    │ - User profile (usage, pattern, priorities)  │  │  │
│  │    │ - Plan details (rate, fees, contract)        │  │  │
│  │    │ - Cost breakdown (annual, savings)           │  │  │
│  │    │ - Ranking position (#1, #2, #3)              │  │  │
│  │    │                                               │  │  │
│  │    │ Prompt structure:                             │  │  │
│  │    │ "You are an expert energy plan advisor...    │  │  │
│  │    │  User Profile: [...]                          │  │  │
│  │    │  Recommended Plan: [...]                      │  │  │
│  │    │  Why ranked #X? Be specific."                │  │  │
│  │    └──────────────────────────────────────────────┘  │  │
│  └────┬─────────────────────────────────────────────────┘  │
│       │                                                     │
│       ▼                                                     │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 4. Call Claude API 🤖                                 │  │
│  │    ┌──────────────────────────────────────────────┐  │  │
│  │    │ anthropic.messages.create({                   │  │  │
│  │    │   model: 'claude-3-5-sonnet-20241022',       │  │  │
│  │    │   max_tokens: 300,                            │  │  │
│  │    │   messages: [{ role: 'user', content }],     │  │  │
│  │    │   signal: abortSignal // 10s timeout         │  │  │
│  │    │ })                                            │  │  │
│  │    │                                               │  │  │
│  │    │ Response time: 500ms-2s                      │  │  │
│  │    │ Cost: ~$0.001 per call                       │  │  │
│  │    └──────────────────────────────────────────────┘  │  │
│  └────┬─────────────────────────────────────────────────┘  │
│       │                                                     │
│       ▼                                                     │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 5. Validate Response                                  │  │
│  │    ┌──────────────────────────────────────────────┐  │  │
│  │    │ Checks:                                       │  │  │
│  │    │ - Length: 50-500 chars ✓                     │  │  │
│  │    │ - Contains numbers ✓                         │  │  │
│  │    │ - Not generic ✓                              │  │  │
│  │    │                                               │  │  │
│  │    │ If invalid → Use fallback template           │  │  │
│  │    └──────────────────────────────────────────────┘  │  │
│  └────┬─────────────────────────────────────────────────┘  │
│       │                                                     │
│       ▼                                                     │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ 6. Cache Result                                       │  │
│  │    explanationCache.set(key, explanation)            │  │
│  │    (Check size limit: 1000 max)                      │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                              │
│  Promise.all() waits for all 3 to complete                 │
│  Total time: ~1-2 seconds (parallel)                       │
└──────────────────────────────────────────────────────────────┘
     │
     ▼
Return [explanation1, explanation2, explanation3]
```

---

## AI Integration Points

### 🤖 AI Layer Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                      AI INTEGRATION LAYER                        │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  Anthropic Claude API Wrapper                           │    │
│  │  (lib/anthropic/)                                       │    │
│  │                                                          │    │
│  │  ┌────────────┐   ┌──────────────┐   ┌──────────────┐ │    │
│  │  │  Client    │   │ Explanations │   │    Batch     │ │    │
│  │  │  Setup     │──>│  Generator   │──>│  Processing  │ │    │
│  │  └────────────┘   └──────────────┘   └──────────────┘ │    │
│  │       │                  │                    │         │    │
│  │       │                  ▼                    ▼         │    │
│  │       │          ┌──────────────┐    ┌──────────────┐ │    │
│  │       │          │    Prompt    │    │ Parallel API │ │    │
│  │       │          │   Builder    │    │    Calls     │ │    │
│  │       │          └──────────────┘    └──────────────┘ │    │
│  │       │                  │                    │         │    │
│  │       ▼                  ▼                    ▼         │    │
│  │  ┌────────────────────────────────────────────────┐   │    │
│  │  │           Cache Layer (In-Memory)              │   │    │
│  │  │  - Key: state-planId-rank-priority             │   │    │
│  │  │  - Max size: 1000 entries                      │   │    │
│  │  │  - Hit rate: ~70-80%                           │   │    │
│  │  └────────────────────────────────────────────────┘   │    │
│  │                         │                               │    │
│  │                         ▼                               │    │
│  │              ┌─────────────────────┐                    │    │
│  │              │  Fallback Templates │                    │    │
│  │              │  (on error/timeout) │                    │    │
│  │              └─────────────────────┘                    │    │
│  └─────────────────────────────────────────────────────────┘    │
│                             │                                    │
└─────────────────────────────┼────────────────────────────────────┘
                              │
                              ▼
                    ┌──────────────────────┐
                    │   Anthropic API      │
                    │   (External Service) │
                    │                      │
                    │   Endpoint:          │
                    │   api.anthropic.com  │
                    │                      │
                    │   Model:             │
                    │   claude-3-5-sonnet  │
                    │   -20241022          │
                    └──────────────────────┘
```

### AI Prompt Engineering

**Prompt Structure:**
```
┌──────────────────────────────────────────────────────────┐
│ SYSTEM CONTEXT                                            │
│ "You are an expert energy plan advisor..."                │
└──────────────────────────────────────────────────────────┘
                        │
                        ▼
┌──────────────────────────────────────────────────────────┐
│ USER PROFILE DATA                                         │
│ - Annual usage: 12,000 kWh                               │
│ - Monthly average: 1,000 kWh                             │
│ - Usage pattern: summer_peak                             │
│ - Current cost: $1,450/year                              │
│ - Priorities: cost (renewable: 0%+, contract: ≤24mo)     │
└──────────────────────────────────────────────────────────┘
                        │
                        ▼
┌──────────────────────────────────────────────────────────┐
│ RECOMMENDED PLAN DETAILS                                  │
│ - Supplier: Texas GreenPower                             │
│ - Plan: 100% Green 12-Month                              │
│ - Rate: $0.1199/kWh                                      │
│ - Monthly fee: $9.95                                     │
│ - Contract: 12 months                                     │
│ - Renewable: 100%                                         │
│ - Rating: 4.3/5.0                                        │
│ - Projected cost: $1,558.80/year                         │
│ - Savings: -$108.80/year (more expensive)                │
└──────────────────────────────────────────────────────────┘
                        │
                        ▼
┌──────────────────────────────────────────────────────────┐
│ SPECIFIC QUESTION                                         │
│ "Why is this plan ranked #1?"                            │
│ "Focus on what matters most based on their priorities."  │
│ "Be specific with numbers. Mention one trade-off."       │
└──────────────────────────────────────────────────────────┘
                        │
                        ▼
┌──────────────────────────────────────────────────────────┐
│ CONSTRAINTS & REQUIREMENTS                                │
│ - 2-3 sentences maximum                                  │
│ - Include specific dollar amounts                         │
│ - Use "you" and "your" (direct address)                  │
│ - No marketing fluff, just facts                         │
└──────────────────────────────────────────────────────────┘
```

**AI Response Processing:**
```
Claude API Response
     │
     ▼
┌────────────────────────┐
│ Extract text content   │
└────┬───────────────────┘
     │
     ▼
┌────────────────────────┐   FAIL   ┌─────────────────────┐
│ Validate explanation   │─────────>│ Use fallback        │
│ - Length OK?           │          │ template            │
│ - Has numbers?         │          │ (deterministic)     │
│ - Not generic?         │          └─────────────────────┘
└────┬───────────────────┘
     │ PASS
     ▼
┌────────────────────────┐
│ Return explanation     │
└────────────────────────┘
```

---

## Client-Server Interactions

### Interaction Pattern 1: Server Components (No JS)

```
USER REQUEST                    SERVER PROCESSING
    │                                │
    │ GET /                          │
    ├──────────────────────────────>│
    │                                │ 1. Render React component
    │                                │ 2. Generate HTML
    │                                │ 3. Send to client
    │<───────────────────────────────┤
    │   HTML (no JavaScript needed)  │
    │                                │
    
Example: Landing page (app/page.tsx)
- No client-side state
- Pure HTML/CSS
- Fast first paint
- SEO friendly
```

### Interaction Pattern 2: Client Components (Hydrated)

```
USER REQUEST                    SERVER PROCESSING
    │                                │
    │ GET /usage                     │
    ├──────────────────────────────>│
    │                                │ 1. Send HTML shell
    │                                │ 2. Send React bundle
    │<───────────────────────────────┤
    │   HTML + JavaScript            │
    │                                │
    │ (Client-side hydration)        │
    │                                │
    │ User enters data               │
    │ Form validation                │
    │ Save to sessionStorage         │
    │ (All happens client-side)      │
    │                                │

Example: Usage input (app/usage/page.tsx)
- Interactive form
- Client-side state
- No server roundtrips
- sessionStorage persistence
```

### Interaction Pattern 3: API Routes

```
CLIENT                          API ROUTE                    SERVICES
  │                               │                             │
  │ POST /api/recommendations     │                             │
  │ {                             │                             │
  │   userId: "...",              │                             │
  │   state: "TX",                │                             │
  │   monthlyUsageKwh: [...],     │                             │
  │   preferences: {...}          │                             │
  │ }                             │                             │
  ├─────────────────────────────>│                             │
  │                               │                             │
  │                               │ Rate limit                  │
  │                               │ Validate                    │
  │                               │                             │
  │                               │ Query DB                    │
  │                               ├────────────────────────────>│
  │                               │   Prisma → Supabase         │
  │                               │<────────────────────────────┤
  │                               │   Plans data                │
  │                               │                             │
  │                               │ Run algorithms              │
  │                               │ (pure functions)            │
  │                               │                             │
  │                               │ Generate explanations       │
  │                               ├────────────────────────────>│
  │                               │   Call Claude API (3×)      │
  │                               │<────────────────────────────┤
  │                               │   Explanations              │
  │                               │                             │
  │                               │ Build response              │
  │                               │                             │
  │<──────────────────────────────┤                             │
  │ {                             │                             │
  │   recommendations: [...],     │                             │
  │   metadata: {...}             │                             │
  │ }                             │                             │
  │                               │                             │
```

### Data Transfer Sizes

```
Request/Response Sizes:

POST /api/recommendations
  Request:  ~0.5 KB  (12 numbers + preferences)
  Response: ~5-10 KB (3 plans + explanations)

GET /api/plans
  Request:  ~0.1 KB  (query params)
  Response: ~10-20 KB (20-50 plans)

GET /plan/[id] (Server Component)
  Request:  ~0.1 KB  (URL param)
  Response: ~2-3 KB  (HTML page)

Total data transfer per user session: ~15-30 KB
```

---

## Database Schema

### Prisma Schema Relationships

```
┌──────────────────────────────────────────────────────────┐
│                          Plan                             │
├──────────────────────────────────────────────────────────┤
│ Field                  Type      Constraints              │
├──────────────────────────────────────────────────────────┤
│ id                     String    @id @default(cuid())    │
│ planId                 String    @unique                 │
│ state                  String    indexed                 │
│ supplierName           String    indexed                 │
│ planName               String                            │
│ rateType               String    'fixed'|'variable'|'tou'│
│ ratePerKwh             Float     indexed                 │
│ monthlyFee             Float     default(0)              │
│ contractLengthMonths   Int?      nullable                │
│ earlyTerminationFee    Float     default(0)              │
│ renewablePct           Int       indexed (0-100)         │
│ supplierRating         Float     (1.0-5.0)               │
│ planDetails            Json?     nullable                │
│ createdAt              DateTime  auto                    │
│ updatedAt              DateTime  auto                    │
└──────────────────────────────────────────────────────────┘

Indexes:
- state (for filtering by geography)
- renewablePct (common user constraint)
- ratePerKwh (cost sorting)
- supplierName (plan lookups)

Query patterns:
WHERE state = ? AND renewablePct >= ? AND contractLengthMonths <= ?
```

### Data Access Patterns

```
Read-Heavy Workload:
┌────────────────────────────────────────┐
│ Query                  │ Frequency     │
├────────────────────────────────────────┤
│ findMany (state)       │ Every request │
│ findUnique (id)        │ Occasional    │
│ findMany (filters)     │ Rare          │
└────────────────────────────────────────┘

Write Operations:
- Seed: One-time (or infrequent manual updates)
- No user writes to Plans table
- No recommendation history stored (MVP)

Connection Pool:
- Prisma manages automatically
- Supabase free tier: 60 connections
- Expected usage: 1-2 concurrent
```

---

## Error Handling Flows

### Error Flow 1: Validation Error

```
Client sends invalid data
     │
     ▼
API Route: POST /api/recommendations
     │
     ▼
┌──────────────────────────┐
│ Zod validation fails     │
│ - Only 11 months?        │
│ - Invalid state?         │
│ - Missing preferences?   │
└────┬─────────────────────┘
     │
     ▼
┌──────────────────────────┐
│ Catch ZodError           │
│ Extract error details    │
└────┬─────────────────────┘
     │
     ▼
┌──────────────────────────┐
│ Return 400 Bad Request   │
│ {                        │
│   error: "Invalid data", │
│   details: [...]         │
│ }                        │
└────┬─────────────────────┘
     │
     ▼
Client displays error message
"CSV must contain exactly 12 months of data"
```

### Error Flow 2: Rate Limit Exceeded

```
Client makes 11th request in 1 minute
     │
     ▼
┌──────────────────────────┐
│ Rate limit check         │
│ - Count: 11              │
│ - Limit: 10              │
│ - Exceeded!              │
└────┬─────────────────────┘
     │
     ▼
┌──────────────────────────┐
│ Return 429 Too Many      │
│ {                        │
│   error: "Too many..."   │
│ }                        │
│ Headers:                 │
│   X-RateLimit-Reset: ... │
└────┬─────────────────────┘
     │
     ▼
Client handles 429
"Too many requests. Please wait a moment."
```

### Error Flow 3: AI Timeout

```
AI call takes >10 seconds
     │
     ▼
┌──────────────────────────┐
│ AbortController triggers │
│ after 10s timeout        │
└────┬─────────────────────┘
     │
     ▼
┌──────────────────────────┐
│ Catch AbortError         │
│ Log: "LLM timeout"       │
└────┬─────────────────────┘
     │
     ▼
┌──────────────────────────┐
│ Use fallback template:   │
│ "This plan can save you  │
│  $X/year with Y%         │
│  renewable energy..."    │
└────┬─────────────────────┘
     │
     ▼
Continue processing
(User never knows AI failed)
```

### Error Flow 4: No Plans Available

```
User selects PA, but no PA plans in DB
     │
     ▼
┌──────────────────────────┐
│ Query returns empty []   │
│ allPlans.length === 0    │
└────┬─────────────────────┘
     │
     ▼
┌──────────────────────────┐
│ Return 503 Service       │
│ {                        │
│   error: "No plans       │
│   available for PA. Try  │
│   different state."      │
│ }                        │
└────┬─────────────────────┘
     │
     ▼
Client displays helpful error
+ "Try selecting Texas instead"
```

---

## Performance Architecture

### Critical Path Analysis

```
User clicks "Get Recommendations"
     │
     ▼
┌─────────────────────────────────────────────────────┐
│ CRITICAL PATH (Target: <2 seconds)                  │
├─────────────────────────────────────────────────────┤
│                                                       │
│ 1. API Request Overhead              10ms            │
│    └─ Network latency                               │
│                                                       │
│ 2. Rate Limiting                     <5ms            │
│    └─ In-memory lookup                              │
│                                                       │
│ 3. Request Validation                5ms             │
│    └─ Zod schema parsing                            │
│                                                       │
│ 4. Database Query                    50-100ms        │
│    └─ Supabase SELECT (indexed)                     │
│                                                       │
│ 5. Usage Analysis                    <5ms            │
│    └─ Array operations                              │
│                                                       │
│ 6. Cost Calculations (20 plans)     10ms            │
│    └─ 20× simple math                               │
│                                                       │
│ 7. Plan Scoring (20 plans)           10ms            │
│    └─ 20× weighted sums                             │
│                                                       │
│ 8. Ranking & Filtering               5ms             │
│    └─ Array sort + slice                            │
│                                                       │
│ 9. AI Explanations (BOTTLENECK)      1-2 seconds ⚠️ │
│    └─ 3× parallel Claude API calls                  │
│    └─ Cache hit: 0ms (70% of time)                 │
│                                                       │
│ 10. Response Serialization           <5ms            │
│     └─ JSON.stringify                               │
│                                                       │
├─────────────────────────────────────────────────────┤
│ TOTAL: 1.1-2.2 seconds (avg: 1.5s)                  │
│ 95th percentile: <2s (meets target)                 │
└─────────────────────────────────────────────────────┘

Optimization opportunities:
1. ✅ Cache AI responses (70-80% hit rate)
2. ✅ Parallel AI calls (saves 1-2s vs sequential)
3. ⚠️ Pre-compute plan costs (for static catalog)
4. ⚠️ Use Claude Haiku (faster, cheaper, but lower quality)
```

### Caching Strategy

```
┌─────────────────────────────────────────────────────────┐
│                    CACHING LAYERS                        │
├─────────────────────────────────────────────────────────┤
│                                                           │
│ 1. LLM Response Cache (In-Memory)                       │
│    Location: lib/anthropic/explanations.ts              │
│    Key: state-planId-rank-priority                      │
│    TTL: Infinite (cleared on restart)                   │
│    Size: Max 1000 entries                               │
│    Hit rate: 70-80%                                     │
│                                                           │
│ 2. Rate Limit Cache (In-Memory)                         │
│    Location: lib/rate-limit.ts                          │
│    Key: IP address                                      │
│    TTL: 60 seconds                                      │
│    Size: Unbounded (cleaned every 5 min in dev)        │
│                                                           │
│ 3. Next.js Page Cache (CDN)                             │
│    - Static pages: Cached at edge                       │
│    - Dynamic pages: Not cached                          │
│    - API routes: Not cached                             │
│                                                           │
│ 4. Database Connection Pool (Prisma)                    │
│    - Reuses connections                                 │
│    - Managed automatically                              │
└─────────────────────────────────────────────────────────┘

Post-MVP caching opportunities:
- Redis for distributed rate limiting
- Redis for LLM response cache (shared across instances)
- Pre-computed plan costs (materialized view)
```

---

## Security Architecture

### Security Layers

```
┌─────────────────────────────────────────────────────────┐
│ 1. CLIENT-SIDE SECURITY                                  │
├─────────────────────────────────────────────────────────┤
│ - Form validation (Zod on client)                       │
│ - Input sanitization (before sessionStorage)            │
│ - No sensitive data in sessionStorage                   │
│ - HTTPS only (enforced by Vercel)                       │
└─────────────────────────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────┐
│ 2. TRANSPORT SECURITY                                    │
├─────────────────────────────────────────────────────────┤
│ - TLS 1.3 (automatic via Vercel)                        │
│ - Certificate: Let's Encrypt (auto-renewed)            │
│ - CORS: Next.js default (same-origin)                   │
└─────────────────────────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────┐
│ 3. SERVER-SIDE SECURITY                                  │
├─────────────────────────────────────────────────────────┤
│ - Rate limiting (10 req/min per IP)                     │
│ - Input validation (Zod schema)                         │
│ - SQL injection prevention (Prisma)                     │
│ - Environment variables (server-only)                   │
│   * ANTHROPIC_API_KEY (never exposed to client)        │
│   * DATABASE_URL (server-only)                          │
└─────────────────────────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────┐
│ 4. DATABASE SECURITY                                     │
├─────────────────────────────────────────────────────────┤
│ - Connection string encryption (Supabase)               │
│ - Row-level security (Supabase, not used in MVP)       │
│ - SSL connections (required)                            │
│ - Prepared statements (Prisma)                          │
└─────────────────────────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────┐
│ 5. API KEY SECURITY                                      │
├─────────────────────────────────────────────────────────┤
│ - Anthropic API key (server-side only)                  │
│ - Supabase anon key (public, RLS-protected)            │
│ - No keys in git (.env.local in .gitignore)            │
│ - Vercel environment variables (encrypted)              │
└─────────────────────────────────────────────────────────┘
```

---

## Deployment Architecture

### Vercel Serverless Architecture

```
┌─────────────────────────────────────────────────────────────┐
│                       VERCEL EDGE NETWORK                    │
│                     (Global CDN + Functions)                 │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │  US East     │  │   US West    │  │   Europe     │      │
│  │  (Primary)   │  │              │  │              │      │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘      │
│         │                  │                  │              │
│         └──────────────────┼──────────────────┘              │
│                            │                                 │
│                    ┌───────┴────────┐                        │
│                    │  Edge Function │                        │
│                    │  (API Routes)  │                        │
│                    └───────┬────────┘                        │
│                            │                                 │
└────────────────────────────┼─────────────────────────────────┘
                             │
                             ▼
              ┌──────────────────────────┐
              │     Supabase             │
              │  (US East - Primary)     │
              │                          │
              │  PostgreSQL Database     │
              └──────────────────────────┘

Function lifecycle:
1. Cold start: 200-500ms (first request)
2. Warm: <50ms overhead
3. Timeout: 10 seconds max (Vercel free tier)
4. Memory: 1024 MB
5. Concurrent: Up to 10 (free tier)
```

---

## Summary

### Key Architectural Decisions

| Decision | Rationale | Trade-off |
|----------|-----------|-----------|
| **Full-stack Next.js** | Single codebase, type safety | Limited backend flexibility |
| **Server Components** | Fast initial load, SEO | Less interactivity |
| **Client Components for forms** | Rich UX, validation | Larger bundle |
| **sessionStorage** | Simple, no DB writes | Lost on close tab |
| **Serverless (Vercel)** | Zero ops, auto-scale | Cold starts, stateless |
| **Supabase PostgreSQL** | Managed, free tier | Vendor lock-in |
| **Prisma ORM** | Type safety, great DX | Abstraction layer |
| **Claude API** | Best explanations | Cost, latency |
| **In-memory cache** | Simple, fast | Not distributed |
| **No authentication (MVP)** | Faster to ship | No personalization |

### AI Value Proposition

```
┌──────────────────────────────────────────────────────────┐
│ Without AI:                                               │
│ - Generic template explanations                          │
│ - Same text for every user                               │
│ - No contextual reasoning                                │
│ - Low trust/understanding                                │
│                                                           │
│ With AI:                                                  │
│ ✓ Personalized explanations                              │
│ ✓ Context-aware (usage pattern, priorities)             │
│ ✓ Specific numbers and trade-offs                       │
│ ✓ Natural language (not formulaic)                      │
│ ✓ Builds trust and confidence                           │
│                                                           │
│ Cost: ~$0.001-0.003 per recommendation                   │
│ Value: 20% conversion uplift, 30% fewer support tickets  │
│ ROI: High (user acquisition cost >> $0.003)             │
└──────────────────────────────────────────────────────────┘
```

### Data Flow Summary

1. **User Input** → sessionStorage (client-side)
2. **Preferences** → sessionStorage (client-side)
3. **API Request** → Server validates & processes
4. **Database Query** → Fetch plans (Supabase)
5. **Algorithms** → Score and rank (server CPU)
6. **AI Generation** → Explain top 3 (Claude API)
7. **Response** → Client displays results
8. **Plan Details** → Server-rendered page

**Total round trips:** 1-2 per user session
**AI calls:** 3 per recommendation (parallelized)
**Database queries:** 1-2 per session

---

**Document Version:** 1.0  
**Last Updated:** November 11, 2025  
**Companion Documents:** Implementation_PRD.md, Implementation_Tasks.md

